...
from .graph import GraphEncoder as LinearGraphEncoder
...
class GraphEmbedding(nn.Module):
    def __init__(self, config, embedding, new_embedding, graph_type='GAT', layer=1, path_list=None,
                 data_path=None):
        super(GraphEmbedding, self).__init__()
        self.graph_type = graph_type
        self.padding_idx = config.pad_token_id
        self.num_class = config.num_labels
        self._size = None  # 添加size属性
        ...
        # 计算size
        self._size = self.original_embedding.num_embeddings + self.new_embedding.num_embeddings - 1
...
class SimpleClassifier(nn.Module):
    ...
    def forward(self, hidden_states):
        pooled_output = hidden_states[:, 0]  # 使用[CLS] token
...
class HTCModel(BertPreTrainedModel):
    ...
    def __init__(self, config, graph_type='GAT', layer=1, path_list=None, data_path=None, depth2label=None,
        ...
        # 添加简单的分类器用于评估
        self.classifier = SimpleClassifier(config.hidden_size, self.num_labels)
    ...
    def init_embedding(self):
        ...
        # prompt
        prompt_embedding = nn.Embedding(2,
    ...
    def get_simple_output(self, batch_data):
        """简化前向传播，用于评估"""
        try:
            # 使用标准BERT前向传播
            outputs = self.bert(
            ...
            print(f"Simple forward error: {e}")
            # 返回默认值
            batch_size = batch_data['input_ids'].shape[0]
            logits = torch.zeros(batch_size, self.num_labels, device=batch_data['input_ids'].device)
            return {'logits': logits}
    ...
    def forward(
            ...
            return_dict=None,
            predict=None,
            use_simple_forward=False  # 添加标志控制使用简化前向传播
    ):
        ...
        ret = {
            ...
            'label_emb': sequence_output.masked_select(
                multiclass_pos.unsqueeze(-1).expand(-1, -1, sequence_output.size(-1))).view(-1,
